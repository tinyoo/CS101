...
//
    ...
    for ( int i = mid - 2; i >= l; --i )
        copy[i] = gcd( Sequence[i+1], Sequence[i] );
    ...
    while ( i >= l )
    {
        ...
        while ( j < r )
        {
            ...
            {
                num += ( Count( copy[i], copy, mid - 1, l ) * Count( copy[j], copy, mid, r ) );
...
//---------------------------------------
int gcd( int x, int y )  // return the gcd pf x and y
{
    if ( y == 0 )
        return x;
    else
...
//------------------------------------------------
int Count(int x, int *copy, int a, int b)  // count the number of x in the array from position a to b
{
    int num = 0;
    if ( a <= b )
    {
