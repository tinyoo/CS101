...
int MinHeap :: Pop()
{
    ...
    {            
        ...
        else if( rchild > size - 1 )
        {                
            if( min_heap[index].weight > min_heap[lchild].weight )
            {
                Node Temp = min_heap[index];
                min_heap[index] = min_heap[lchild];
                min_heap[lchild] = Temp;                  
                index = lchild;                 
            }                
            else                    
                break;            
        ...
        {
            int smaller;
            if ( min_heap[lchild].weight <= min_heap[rchild].weight )
                smaller = lchild;
            else
                smaller = rchild;             
            if( min_heap[index].weight > min_heap[smaller].weight )
            {
                Node Temp = min_heap[index];
                min_heap[index] = min_heap[smaller];
                min_heap[smaller] = Temp;                
                index = smaller;                 
            }                
            else                    
                break;            
